...
@Suppress("unused") // invoked from generated bytecode
fun configureNestedModel(target: Any, name: String, configure: Action<in Any>) {
    val methodName = "get${name.replaceFirstChar { if (it.isLowerCase()) it.titlecase(getDefault()) else it.toString() }}"
    val nestedModel = target::class.java.getMethod(methodName).invoke(target)
    configure.invoke(nestedModel)
}
